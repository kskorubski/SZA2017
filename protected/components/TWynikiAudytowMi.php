 ...
 * @author studio
 */
 class TWynikiAudytowMi extends CApplicationComponent{     
        var $model_ankieta;
        var $model_etykieta; //model etykiety bylby tu nie potrzebny poniewaz w wersji 2017
                            //nie bierze on udzialu w obliczeniach. Ale zeby nie komplikowac 
                            //i nie przerabiac calej metody i parametrow wej. zostaly usuniete 
                            //poznizej obliczenia zwiazane z dodatniem punktacji z etykety
                            //powstala nowa operacja dla etykiety determinujaca dalszy bieg audytu
        var $metoda;
//1. Pozycjonowanie        
...
//4. Etykieta
        var $ety_uzyskana;
        var $ety_etykieta;
        var $ety_dane_na_etykiecie;
        var $ety_parametry_ekspozycji;
...
//podsumowanie
        ...
        const MAX_RAZEM_CYFRA = 60; //v 2017 nie wliczamy punktow za etykiete bylo 75 - 15 za etykiete = 60
        const MAX_RAZEM_ANALOG = 102; //v 2017 nie wliczamy punktow za etykiete bylo 126 - 24 za etykiete = 102
...
//wagi w wersji 2017 usunieto wagi. Zaliczenie odbywa sie tylko na podstawie uzyskanej punktacji
        const zaPozycjonowanie = 0; //bylo 6
        const zaArtefakty = 0; //bylo 5
        const zaInne = 0; //bylo 5
        const zaEtykiete = 0; //bylo3
//progi zaliczeniowe w pkt.
        ...
        public function setData($model_ankieta,$model_etykieta,$metoda) {
        ...
        public function policz(){
               ...
               $this->sumator();
            $this->progZalPoz = (0.6 * $this->POZ_SUMA_MAX_L); //tylko dla L zrobione bo niema sensu P jest taka sama.
            $this->progZalInne = (0.6 * $this->INNE_MAX_L);
            
            switch ($this->metoda){
                case 'cyfrowa':   
                    $this->progZalArt = (0.65 * $this->ART_MAX_CYFRA_L);
        ...
        function sumator() {
            ...
            if($this->metoda == 'cyfrowa'){
                 ...
                 $this->ety_dane_na_etykiecie = $this->model_etykieta->x4_1+$this->model_etykieta->x4_2+$this->model_etykieta->x4_3+$this->model_etykieta->x4_4+
                         $this->model_etykieta->x4_5+$this->model_etykieta->x4_6+$this->model_etykieta->x4_7+$this->model_etykieta->x4_8+$this->model_etykieta->x4_9;
                 
                 $this->ety_parametry_ekspozycji = $this->model_etykieta->x4_10+$this->model_etykieta->x4_11+$this->model_etykieta->x4_12+$this->model_etykieta->x4_13+$this->model_etykieta->x4_14+
                           $this->model_etykieta->x4_15;
                ...
                $this->ety_etykieta = $this->model_etykieta->x4_1+$this->model_etykieta->x4_2+$this->model_etykieta->x4_3+$this->model_etykieta->x4_4;
                
                $this->ety_dane_na_etykiecie = $this->model_etykieta->x4_5+$this->model_etykieta->x4_6+$this->model_etykieta->x4_7+$this->model_etykieta->x4_8+$this->model_etykieta->x4_9+
                        $this->model_etykieta->x4_10+$this->model_etykieta->x4_11+$this->model_etykieta->x4_12+$this->model_etykieta->x4_13+$this->model_etykieta->x4_14;
                
                $this->ety_parametry_ekspozycji = $this->model_etykieta->x4_15+$this->model_etykieta->x4_16+$this->model_etykieta->x4_17+$this->model_etykieta->x4_18+$this->model_etykieta->x4_19+$this->model_etykieta->x4_20;
            }
        ...
        function getEty_etykieta() {
            return $this->ety_etykieta;
        }
        function getEty_dane_na_etykiecie() {
            return $this->ety_dane_na_etykiecie;
        }
        function getEty_parametry_ekspozycji() {
            return $this->ety_parametry_ekspozycji;
        }
        ...
        function getArt_uzyskana_percent_L() {
            if($this->metoda == 'analogowa') {
                $art_max = self::ART_MAX_ANALOG_L;
            }else if ($this->metoda == 'cyfrowa') {
                $art_max = self::ART_MAX_CYFRA_L;
            }
            $this->art_uzyskana_percent_L = ($this->art_uzyskana_L/$art_max)*100;
        ...
        function getArt_uzyskana_percent_P() {
            ...
            $this->art_uzyskana_percent_P = ($this->art_uzyskana_P/$art_max)*100;
        ...
        function getEty_uzyskana_percent() {
            if($this->metoda == 'analogowa') {
                $ety_max = self::ETY_MAX_ANALOG;
            }else if ($this->metoda == 'cyfrowa') {
                $ety_max = self::ETY_MAX_CYFRA;
            }
            $this->ety_uzyskana_percent = ($this->ety_uzyskana/$ety_max)*100;
        ...
        function getUzyskana_razem_wagi() {
            ...
            //v 2017 nie ddoliczamy punktow z etykiety do ogolnej sumy punktacji
//            if($this->ety_uzyskana >= $this->progZalEtykieta){
...
//            }
        ...
        function getLiczbaZaliczonychParametrow() {
            $licznik = 0;
            if($this->getPoz_zaliczono_L() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getPoz_zaliczono_P() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getArt_zaliczono_L() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getArt_zaliczono_P() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getInne_zaliczono_L() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getInne_zaliczono_P() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            if($this->getEty_zaliczono() == "<div id='green'>TAK</div>"){
                $licznik++;
            }
            return $licznik;
        }
        
        function getCzySpelnioneUtkanie() {
            if (($this->getUtkanie_L() == 'Spełnione kryterium utkania') & ($this->getUtkanie_P() == 'Spełnione kryterium utkania')){
                return 'Spełnione';
            }else {
                return "Niespełnione";
            }
